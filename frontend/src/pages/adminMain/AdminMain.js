...
export const AdminMain = (props) => {
  ...
  console.log(baseURL + `/moderator/get/news`);
  console.log(`${cookiesObject.type} ${cookiesObject.token}`);
  useEffect(() => {
    if (state.show == "news") {
      axios
        .get(baseURL + `/moderator/get/news`, { headers: {
    ...
    if (state.show == "project") {
      axios
        ...
        .then((res) => {
          console.log(res.data);
          setState({ ...state, publList: res.data });
          setState({ ...state, publListCon: res.data.project });
        })
        .catch((er) => {
          console.log(er);
        });
